...
public class CoursService implements IService<Cours> {
    ...
    public List<Cours> readAllSortedByLikes() throws SQLException {
        List<Cours> courses = new ArrayList<>();
        String query = "SELECT * FROM course ORDER BY nbr_like DESC";
        Statement st = cnx.createStatement();
        ResultSet rs = st.executeQuery(query);

        while (rs.next()){
            courses.add(mapCours(rs));
        }
        return courses;
    }

    public List<Cours> readAllSortedByDownloads() throws SQLException {
        List<Cours> courses = new ArrayList<>();
        String query = "SELECT * FROM course ORDER BY nombre_de_telechargements DESC";
        Statement st = cnx.createStatement();
        ResultSet rs = st.executeQuery(query);
    ...
    private Cours mapCours(ResultSet rs) throws SQLException {
        ...
        return new Cours(id, titre, description, contenu, author_id, image, nbr_like, nombre_de_telechargements, video);
    }
    ...
    public void incrementDownloadCount(int coursId) throws SQLException {
        String sql = "UPDATE course SET nombre_de_telechargements = nombre_de_telechargements + 1 WHERE id = ?";
    ...
    public void incrementLikes(int coursId) throws SQLException {
        String sql = "UPDATE course SET nbr_like = nbr_like + 1 WHERE id = ?";
        try (PreparedStatement stmt = cnx.prepareStatement(sql)) {
            stmt.setInt(1, coursId);
            stmt.executeUpdate();
        }
